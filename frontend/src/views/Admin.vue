<template>
  <div class="max-w-3xl mx-auto px-4 py-10">
    <h2 class="text-3xl font-semibold text-orange-400 mb-1">Admin Dashboard</h2>
    ...
    <div class="grid grid-cols-2 gap-4 mb-8" v-if="stats">
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-orange-100 text-center">
        <p class="text-3xl font-semibold text-orange-400">{{ stats.total_users }}</p>
        <p class="text-gray-400 text-sm mt-1">👥 Total Users</p>
      </div>
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-orange-100 text-center">
        <p class="text-3xl font-semibold text-orange-400">{{ stats.total_ratings }}</p>
        <p class="text-gray-400 text-sm mt-1">😊 Total Ratings</p>
      </div>
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-orange-100 text-center">
        <p class="text-3xl font-semibold text-orange-400">{{ stats.total_active_cards }}</p>
        <p class="text-gray-400 text-sm mt-1">🃏 Active Cards</p>
      </div>
      <div class="bg-white rounded-2xl p-6 shadow-sm border border-orange-100 text-center">
        <p class="text-3xl font-semibold text-orange-400">{{ stats.average_mood_rating }}</p>
        <p class="text-gray-400 text-sm mt-1">⭐ Avg Mood</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-orange-100 mb-6">
      ...
      <input v-model="newCard.title" placeholder="Title" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-300 mb-3" />
      <input v-model="newCard.message" placeholder="Message" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-300 mb-3" />
      <input v-model="newCard.category" placeholder="Category (optional)" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none focus:border-orange-300 mb-4" />
      <button @click="submitCard" class="w-full bg-orange-400 text-white py-3 rounded-full text-sm hover:bg-orange-500 transition">
        Add Card ✨
      </button>
      <p v-if="cardSuccess" class="text-green-400 text-xs text-center mt-2">Card added! ✅</p>
    </div>
